...
from pythonrecreation.settings import MEDIA_ROOT, MEDIA_URL
...
def main(request):
    ...
    return render_to_response("forum/list.html", add_csrf(request, forums=forums))
...
def post(request, ptype, pk):
    """Display a post form."""
    action = reverse("pythonrecreation.forum_views.%s" % ptype, args=[pk])
    if ptype == "new_thread":
...
def add_csrf(request, **kwargs):
    ...
    d = dict(user=request.user, is_auth = request.user.is_authenticated(), **kwargs)
